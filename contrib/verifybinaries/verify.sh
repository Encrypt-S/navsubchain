...
###   This script attempts to download the signature file SHA256SUMS.asc from subchain.org
...
WORKINGDIR="/tmp/subchain"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://subchain.org/bin/"
VERSIONPREFIX="subchain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'subchain-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the SubChain Core binary release signing key installed?"
   fi

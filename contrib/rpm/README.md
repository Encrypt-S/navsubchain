...
The RPM spec file provided here is for SubChain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://subchain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[SubChain git repository](https://github.com/subchain/subchain) but are not part of
...
Currently only `Source10` is used, for the example `subchain.conf` file.
...
in the subchain git repository. At present only an SVG version of the SubChain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the subchain
...
needed when 0.12.1 is released, a proper fix is already in the SubChain git
master branch.
...
idea to build SubChain Core against a static version of that library compiled
...
The `subchain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `subchain-server` package. At this point, most RPM based
...
The `subchain-server` package should have SELinux support. How to properly do
...
SubChain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/subchain/). He has been
packaging SubChain for Fedora at least since 2012.
...
4. I renamed the `subchain` package that contains the Qt GUI to `subchain-core` as
that appears to be how the general population refers to it, in contrast to
`subchain-xt` or `subchain-classic`. I wanted to make sure the general population

...
// Copyright (c) 2017 The SubChain developers
 ...
 * between subchaind, and subchain-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SUBCHAIN_CONF_FILENAME));
    if (mode == HMM_SUBCHAIND)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SUBCHAIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/subchaindev/subchain-core>";
    const std::string URL_WEBSITE = "<https://subchain.org>";
...
/** Sanity checks
 *  Ensure that SubChain is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single SubChain process is using the data directory.
    ...
    if(GetBoolArg("-staking", true))
        threadGroup.create_thread(boost::bind(&SubChainStaker, boost::cref(chainparams)));
#endif

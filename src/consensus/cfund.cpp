...
bool CFund::IsValidPaymentRequest(CTransaction tx)
{
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nAmount) + "SUB from the proposal " + proposal.hash.ToString() + ". Payment request id: " + strDZeel;

    CSubChainAddress addr(proposal.Address);
    CKeyID keyID;
    addr.GetKeyID(keyID);

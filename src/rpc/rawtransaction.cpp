                      ...
                      int nHeight = 0, int nConfirmations = 0, int nBlockTime = 0)
{
    ...
    entry.push_back(Pair("tx-comment", tx.strDZeel));
        ...
        if (tx.IsCoinBase())
            ...
            if (GetSpentIndex(spentKey, spentInfo)) {
                ...
                if (spentInfo.addressType == 1) {
                    in.push_back(Pair("address", CSubChainAddress(CKeyID(spentInfo.addressHash)).ToString()));
                } else if (spentInfo.addressType == 2)  {
                    in.push_back(Pair("address", CSubChainAddress(CScriptID(spentInfo.addressHash)).ToString()));
                }
            }
...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"subchainaddress\"        (string) subchain address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "createrawtransaction [{\"txid\":\"id\",\"vout\":n},...] {\"address\":amount,\"data\":\"hex\",...} [tx-comment] ( locktime )\n"
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the subchain address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
            "      \"data\": \"hex\",     (string, required) The key is \"data\", the value is hex encoded data\n"
            "      ...\n"
            "    }\n"
            "3. \"tx-comment\"  (string, optional) Encrypted destination address if you're sending a SUBtech transaction \n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) subchain address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
            "     ,...\n"
            "  ],\n"
            "  \"tx-comment\" : \"id\",             (string) Anon destination\n"
            "}\n"
...
UniValue decodescript(const UniValue& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CSubChainAddress(CScriptID(script)).ToString()));
    return r;
}

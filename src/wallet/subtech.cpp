...
UniValue SUBtech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    UniValue addresses = find_value(serverData, "SUB_addresses");
    UniValue addrArray = addresses.get_array();

    UniValue SUBtechData;
    SUBtechData.setObject();
    SUBtechData.pushKV("anondestination", encryptedAddress);
    SUBtechData.pushKV("public_key", pubKey);
    SUBtechData.pushKV("anonaddress", addrArray);
    SUBtechData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    SUBtechData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return SUBtechData;
  } else {
    throw runtime_error("Unable to send SUBTech transaction, please try again.");
  }

}

vector<anonServer> SUBtech::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one SUBTech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid SUBTech server to send private payments.");
      return returnServers;
  }
...
UniValue SUBtech::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured SUBTech nodes are available right now.");
  }
  ...
  if (curl) {
    ...
    curl_easy_cleanup(curl);
    curl_global_cleanup();
    throw runtime_error("CURL uSUBailable");
  }
}

UniValue SUBtech::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string SUBtech::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",SUBTECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int SUBtech::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * SUBtech::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool SUBtech::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue SUBtech::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue SUBtechData;
        SUBtechData.setObject();
        SUBtechData.pushKV("min_amount", find_value(data, "min_amount"));
        SUBtechData.pushKV("max_amount", find_value(data, "max_amount"));
        SUBtechData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return SUBtechData;
      }

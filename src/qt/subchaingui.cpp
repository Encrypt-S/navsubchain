...
const std::string SubChainGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SubChainGUI::DEFAULT_WALLET = "~Default";

SubChainGUI::SubChainGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void SubChainGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/subchain"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SubChain address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and subchain: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SubChain addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SubChain addresses"));
    ...
    openAction->setStatusTip(tr("Open a subchain: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible SubChain command-line options").arg(tr(PACKAGE_NAME)));
...
void SubChainGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(SubChainUnits::Unit u, SubChainUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(SubChainUnits::name(u)), this);
...
void SubChainGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void SubChainGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        topMenuLogo->setObjectName("SUBLogo");
        topMenuLogo->move(0,0);
        topMenuLogo->setStyleSheet(
           "#SUBLogo { border-image: url(:/icons/menu_logo)  0 0 0 0 stretch stretch; border: 0px; }");
        ...
        // ImageButton* SUBLogo2 = new ImageButton();
        // SUBLogo2->setFixedSize(64,64);
        // SUBLogo2->setObjectName("SUBLogo");
        // SUBLogo2->setPixmap(pixmap);
        //
        // toolbar->setMovable(false);
        // toolbar->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        // toolbar->addWidget(SUBLogo);
        // toolbar->addWidget(SUBLogo2);
...
void SubChainGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool SubChainGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void SubChainGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool SubChainGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SubChainGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SubChainGUI::setWalletActionsEnabled(bool enabled)
{
...
void SubChainGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SubChainGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SubChainGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SubChainGUI::optionsClicked()
{
...
void SubChainGUI::showDebugWindow()
{
...
void SubChainGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SubChainGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SubChainGUI::openClicked()
{
...
void SubChainGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void SubChainGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SubChainGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SubChainGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SubChainGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SubChainGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SubChainGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void SubChainGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to SubChain network", "", count));
    if(walletFrame){
...
void SubChainGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to SubChain network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to SubChain network..."));
...
void SubChainGUI::showVotingDialog()
{
...
void SubChainGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("SubChain"); // default title
...
void SubChainGUI::changeEvent(QEvent *e)
{
...
void SubChainGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SubChainGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SubChainGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SubChainUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SubChainGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SubChainGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SubChainGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SubChainGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SubChainGUI::setEncryptionStatus(int status)
{
...
void SubChainGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SubChainGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SubChainGUI::detectShutdown()
{
...
void SubChainGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SubChainGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(SubChainGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SubChainGUI::subscribeToCoreSignals()
{
...
void SubChainGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<SubChainUnits::Unit> units = SubChainUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const SubChainUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(SubChainUnits::name(unit)));
    }
...
{
    setText(SubChainUnits::name(newUnits));
}
...
void SubChainGUI::toggleStaking()
{
...
void SubChainGUI::updatePrice()
{
  ...
  request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/SUB-coin/?convert=EUR"));
...
void SubChainGUI::replyFinished(QNetworkReply *reply)
{
...
void SubChainGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CSubChainAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void SubChainGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.subchain.org/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void SubChainGUI::updateStakingStatus()
{
    updateWeight();
